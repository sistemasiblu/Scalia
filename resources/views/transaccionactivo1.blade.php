...
<head>
  ...
  <script>
    ...
    function abrirModalCamposCompania()
    {
...
function abrir(){ 
  ...
  popup.focus();
    ...
    var transaccioncompania = '<?php echo isset($transaccionCompania)? json_encode($transaccionCompania) : "";?>';
    transaccioncompania = (transaccioncompania != '' ? JSON.parse(transaccioncompania) : '');
...
$(document).ready(function()
{
  ...
  encabezado.campos=['idTransaccionActivoCampoE', 'CampoTransaccion_idCampoTransaccionE', 'descripcionCampoTransaccionE','obligatorioTransaccionActivoCampoE'];
  encabezado.etiqueta=['input','input','input','checkbox'];
  encabezado.tipo=['hidden','hidden','','checkbox'];
  encabezado.estilo=['','','width:200px; height:35px;','width:40px; height:31px;display: inline-block;'];
  encabezado.clase=['','','',''];
  encabezado.sololectura=[false,false,true];
  encabezado.completar=['off','off','off'];
  encabezado.opciones = [[],[],[]];      
...
$(document).ready(function()
{
  ...
  detalle.campoid = 'idTransaccionActivoCampoD';
  detalle.campoEliminacion = 'detalleEliminar';
  detalle.campos=['idTransaccionActivoCampoD','CampoTransaccion_idCampoTransaccionD','descripcionCampoTransaccionD',  'obligatorioTransaccionActivoCampoD'];
  detalle.etiqueta=['input','input','input','checkbox'];
  detalle.tipo=['hidden','hidden','text','checkbox'];
  detalle.estilo=['','','width:200px; height:35px;','width:40px; height:31px;display: inline-block;'];
  detalle.clase=['','','',''];
  detalle.sololectura=[false,false,true,false];
  detalle.completar=['off','off','off','off'];
  detalle.opciones = [[],[],[],[]];      
  detalle.funciones=['','','',''];
  ...
  for(var j=0; j < transaccionDetalle.length; j++)
  {
      detalle.agregarCampos(JSON.stringify(transaccionDetalle[j]),'L');
  }
...
var valorcompania = [0,0,''];
$(document).ready(function()
{

  compania=new Atributos('compania','contenedor-compania','compania-');
  compania.campoid = 'idTransaccionCompania';
  compania.campoEliminacion = 'companiaEliminar';
  compania.campos=['Compania_idCompania','nombreCompania', 'idTransaccionCompania'
    ];
  compania.etiqueta=['input','input','input'];
  compania.tipo=['hidden','','hidden'];
  compania.estilo=['','width:200px; height:35px;',''];

  compania.clase=['','',''];
  compania.sololectura=[false,false,false];
  compania.completar=['off','off','off'];
  ...
  for(var j=0; j < transaccioncompania.length; j++)
  {
      compania.agregarCampos(JSON.stringify(transaccioncompania[j]),'L');
  }
...
<div class='form-group'>
 {!!Form::label('codigoTransaccionActivo', 'Codigo', array('class' => 'col-sm-2 control-label')) !!}
    <div class="col-sm-6">
      ...
      {!!Form::hidden('companiaEliminar', null, array('id' => 'companiaEliminar')) !!}
...
<div class="tab-content" id="contenidopestanas">
  <div class="tab-pane fade in active" id="cpestana1">
    <div class="container"><br><br><br>
      <div id="pestanas">
        <ul id=lista class="nav nav-tabs">
          <li class="active" id="pestana1"><a data-toggle="tab" href='#pestEncabezado'>Encabezado</a></li>
          <li id="pestana2"><a data-toggle="tab" href='#pestDetalle'>Detalle</a></li>
        </ul>
      </div>
    </div>

    <div class="tab-content" id="contenidopestanasDatos">
      <div class="tab-pane fade in active" id="pestEncabezado">
       <br>
      ...
      </div><!--Fin Div pestEncabezado-->

      <div class="tab-pane fade" id="pestDetalle" >
       <br>
        <div class="form-group">
          <fieldset id='varioslistachequeo-form-fieldset'>
            <div class="form-group"  id='test'>
              <div class="col-sm-12">
                <div class="row show-grid">
                  <div class="col-md-1" style="width: 40px;height: 35px;" >
                    <span class="glyphicon glyphicon-plus" onclick="abrirModalCamposDetalle();"></span> 
                  </div>
                  <div class="col-md-1" style="width: 200px;height: 35px;"><b>Nombre del Campo</b></div>
                  <div class="col-md-1" style="width: 100px;height: 35px; "><b>Obligatorio</b></div>
  ...
  <div class="tab-pane fade " id="cpestana3">
    <div class="container"><br><br><br>
      <div id="pestanas">
        <ul id=lista class="nav nav-tabs">
          <li class="active" id="pestana1"><a data-toggle="tab" href='#pestCompania'>Compañias</a></li>
          <li id="pestana2"><a data-toggle="tab" href='#pestRol'>Roles</a></li>
        </ul>
      </div>
    </div>

    <div class="tab-content" id="contenidopestanasDatos">
      <div class="tab-pane fade in active" id="pestCompania">
      <br><br>
          <div class="form-group">
          <fieldset id='varioslistachequeo-form-fieldset'>
            <div class="form-group"  id='test'>
              <div class="col-sm-12">
                <div class="row show-grid">
                  <div class="col-md-1" style="width: 40px;height: 35px;" >
                    <span class="glyphicon glyphicon-plus" onclick="abrirModalCamposCompania();"></span> 
                  </div>
                  <div class="col-md-1" style="width: 200px;height: 35px;"><b>Compañia</b></div>
                  <div id="contenedor-compania"></div>
                </div>      
              </div>
            </div>
          </fieldset>
        </div>
      </div><!--Fin Div pestCompania-->
      <div class="tab-pane fade" id="pestRol" >
        <br><br>
          <div class="form-group">
          <fieldset id='varioslistachequeo-form-fieldset'>
            <div class="form-group"  id='test'>
    ...
    </div><!--Fin Div contenidopestanasDatos -->
...
<div id="myModalCompania" class="modal fade" role="dialog" style="display: none;">
  <div class="modal-dialog" style="width:70%;">
    <div style="" class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        ...
        <div class="modal-body">
          <?php 
          echo '<iframe style="width:100%; height:400px; " id="campos" name="campos" src="http://'.$_SERVER["HTTP_HOST"].'/companiagridselect"></iframe>'
          ?>
        </div>
    </div>
  </div>
