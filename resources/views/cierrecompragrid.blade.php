...
@section('titulo')<h3 class="pestana" id="titulo"><center>Cierre de compras</h3>@stop
@section('content')
{!!Form::model($datos)!!}
<style>
    tfoot input {
                    ...
                    <table id="tcierrecompra" name="tcierrecompra" class="display table-bordered" width="100%">
                        <thead>
                            <tr class="btn-primary active">
                                ...
                                <a href="cierrecompra/create"><span style= "display: <?php echo $visible;?> color:white;" class="glyphicon glyphicon-plus"></span></a>
...
<script type="text/javascript">
    ...
    $(document).ready( function () {
        ...
        /*$('#tcierrecompra').DataTable({
            "aProcessing": true,
            "aServerSide": true,
            "stateSave":true,
            "ajax": "{!! URL::to ('/datosCierreCompra')!!}",
        });*/
        var lastIdx = null;
        var modificar = '<?php echo (isset($datos[0]) ? $dato["modificarRolOpcion"] : 0);?>';
        var eliminar = '<?php echo (isset($datos[0]) ? $dato["eliminarRolOpcion"] : 0);?>';
        var table = $('#tcierrecompra').DataTable( {
            "order": [[ 1, "asc" ]],
            "aProcessing": true,
            "aServerSide": true,
            "stateSave":true,
            "ajax": "{!! URL::to ('/datosCierreCompra?modificar="+modificar+"&eliminar="+eliminar+"')!!}",
            "language": {
    ...
    $('#tcierrecompra tfoot th').each( function () {
        if($(this).index()>0){
        var title = $('#tcierrecompra thead th').eq( $(this).index() ).text();
