...
@section('titulo')<h3 class="pestana" id="titulo"><center>Dispositivo</center></h3>@stop
@section('content')
<style>
    tfoot input {
                width: 100%;
        ...
        <div class="container">
            <div class="row">
                <div class="container">
                    ...
                    <table id="tdispositivo" name="tdispositivo" class="display table-bordered" width="100%">
                        <thead>
                            <tr class="btn-primary active">
                                <th style="width:60px;padding: 1px 8px;" data-orderable="false">
                                <a href="dispositivo/create"><span style= "display: <?php echo $visible;?> color:white;" class="glyphicon glyphicon-plus"></span></a>
...
<script type="text/javascript">
    ...
    $(document).ready( function () {
        ...
        /*$('#tdispositivo').DataTable({
            "aProcessing": true,
            "aServerSide": true,
            "stateSave":true,
            "ajax": "{!! URL::to ('/datosDispositivo')!!}",
        });*/
        var lastIdx = null;
        var modificar = '<?php echo (isset($datos[0]) ? $dato["modificarRolOpcion"] : 0);?>';
        var eliminar = '<?php echo (isset($datos[0]) ? $dato["eliminarRolOpcion"] : 0);?>';
        var table = $('#tdispositivo').DataTable( {
            "order": [[ 1, "asc" ]],
            "aProcessing": true,
            "aServerSide": true,
            "stateSave":true,
            "ajax": "{!! URL::to ('/datosDispositivo?modificar="+modificar+"&eliminar="+eliminar+"')!!}",
            "language": {
    ...
    $('#tdispositivo tfoot th').each( function () {
        if($(this).index()>0){
        var title = $('#tdispositivo thead th').eq( $(this).index() ).text();
