...
<title>Importacion detallada listado</title>
@section('contenido')
	  ...
	  while ($s < $total and $documento == $datos[$s]["nombreDocumentoImportacion"])
		{
			...
			while ($s < $total and $documento == $datos[$s]["nombreDocumentoImportacion"] and $cliente == $datos[$s]['nombreClienteCompra']) 
			{
				...
				#Sumo los valores por clientes
					$volumenCliente += $datos[$s]['volumenFacturaEmbarqueDetalle'];
					$cantidadPedidoCliente += $datos[$s]['cantidadPedido'];
					$cantidadFacturaCliente += $datos[$s]['cantidadFactura'];
					$cantidadPendienteCliente += $datos[$s]['cantidadPendiente'];
					$cantidadCumplimientoCliente += $datos[$s]['cumplimientoCantidad'];
					$cantidadDatosCliente += count($cliente);
					$valorPedidoCliente += $datos[$s]['valorPedido'];
					$valorFacturaCliente += $datos[$s]['valorFactura'];
					$valorPendienteCliente += $datos[$s]['valorPendiente'];
					$valorCumplimientoCliente += $datos[$s]['cumplimientoValor'];
		...
		$totalCantidadDatos += count($cantidadDatosCliente);
