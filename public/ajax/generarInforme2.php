				...
				foreach ($tipoContabilidad as $tc => $tipoCont) 
				{
					for($anio = 0; $anio <= 1; $anio++)
					{
						...
						foreach ($sistemaInformacion as $db => $baseDatos) 
						{
							...
							for($regcon = 0; $regcon < count($consulta); $regcon++)
							{
								// $consulta[$regcon]['Porc_'.$baseDatos.'_'.$mes] = 0;
								// $consulta[$regcon]['Cons_'.$mes] = 0;

								$consulta = calcularPorcentajeVerticalBasadoFormula($informecapa["valorInformeConcepto"], $mes, $baseDatos, $consulta, $idInformeCapa, $consulta[$regcon][$baseDatos.'_'.$mes]);
...
function calcularPorcentajeVerticalBasadoFormula($formula, $mes, $baseDatos, $consulta, $idInformeCapa, $valorFormula)
{
	// las formulas tienen variables o valores fijos los cuales estan separados
    // por signos o parentesis "+ - * / ( )", todo lo que se encuentre entre 2 signos lo 
    // trataremos como un valor a calcular o valor fijo
    ...
    while($formula != '' and $pos <= $total)
    {
    	...
    	if($letra == '+' or $letra == '-' or $letra == '*' or $letra == '/' or $letra == '(' or $letra == ')')
    	{
			...
			for($buscar = 0; $buscar < count($consulta); $buscar++)
			{
				...
				{
					$consulta[$buscar]['Porc_'.$baseDatos.'_'.$mes] = $consulta[$buscar][$baseDatos.'_'.$mes] / ($valorFormula == 0 ? 1 : $valorFormula) * 100;
					
					if(isset($consulta[$buscar]['Cons_'.$mes]))
						$consulta[$buscar]['Porc_Cons_'.$mes] = $consulta[$buscar]['Cons_'.$mes] / ($valorFormula == 0 ? 1 : $valorFormula) * 100;
				}
			}
			$formula = substr($formula, $aux+1);
    		$aux = 0;
		...
		($formato == 'Res' and $consulta[$i]["resumen"] == 1))
		{
			...
			while($inicio < $fechaFinal)
			{
				...
				foreach ($tipoContabilidad as $tc => $tipoCont) 
				{
						...
						if(($colPorcentajeVert != '' or $colPorcentajeFormula != ''))
							$detalleInforme .= '<td '.$estiloNumero.'>'.($MC ? 'Porc_'.$baseDatos.'_'.$mes.'=' : '').
								(isset($consulta[$i]['Porc_'.$baseDatos.'_'.$mes]) 
									? ($consulta[$i]["tipoValor"] == 'Titulo' ? '&nbsp;' :number_format($consulta[$i]['Porc_'.$baseDatos.'_'.$mes],2,'.',',')).'%' 
									: '&nbsp;')
								.'</td>'; 
					...
					if($consolidar)
					{
						$detalleInforme .= '<td '.$estiloNumero.'>'.($MC ? 'Cons_'.$mes : '').($consulta[$i]["tipoValor"] == 'Titulo' ? '&nbsp;' :number_format($consulta[$i]['Cons_'.$mes],2,'.',',')).'</td>';

						if($colPorcentajeVert != '' or $colPorcentajeFormula != '')
						{
							if(isset($consulta[$i]['Porc_Cons_'.$mes]))
								$detalleInforme .= '<td '.$estiloNumero.'>'.($MC ? 'Porc_Cons_'.$mes.'=' : '').($consulta[$i]["tipoValor"] == 'Titulo' ? '&nbsp;' :number_format($consulta[$i]['Porc_Cons_'.$mes],2,'.',',').'%' ).'</td>'; 
							else
