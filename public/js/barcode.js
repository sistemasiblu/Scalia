...
(function ($) {
  ...
  var barcode = {
    ...
    // clear jQuery Target
    resizeImg: function($container, w){
      $container
        .css("padding", "0px")
        .css("overflow", "hidden")
        .html("");
    ...
    digitToBmp2D: function($container, settings, digit, hri){
      ...
      this.digitToBmpRenderer($container, settings, digit, hri, s, s);
        ...
        // img barcode renderer
    digitToImgRenderer: function($container, settings, digit, hri, mw, mh){
      var lines = digit.length;
      var columns = digit[0].length;
      var i = 0;
      var c0 = this.isHexColor(settings.bgColor) ? this.lec.cHexColor(settings.bgColor) : this.lec.cRgb(255,255,255);
      ...
      // set img image to the container
      var object = document.createElement('img');
      object.setAttribute('width', '100%');
      object.setAttribute('src', 'data:image/bmp;base64,'+ this.base64Encode(img));
      this.resizeImg($container, mw * columns + padding).append(object);
                      
    },
    // img 1D barcode renderer
    digitToImg: function($container, settings, digit, hri){
      var w = barcode.intval(settings.barWidth);
      var h = barcode.intval(settings.barHeight);
      this.digitToImgRenderer($container, settings, this.bitStringTo2DArray(digit), hri, w, h);
    },
    // img 2D barcode renderer
    digitToImg2D: function($container, settings, digit, hri){
      var s = barcode.intval(settings.moduleSize);
      this.digitToImgRenderer($container, settings, digit, hri, s, s);
    },
