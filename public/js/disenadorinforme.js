...
var totalGrupos = new Array();
var colores = new Array(); 

function generarColorPastel()
{
    // var valor = Math.floor(Math.random() * 100);
    // var pastel = 'hsl(' + valor + ', 100%, 87.5%)';


    // ----------------------------------------------
    // Libreria JS randomColor
    // https://github.com/davidmerfield/randomColor
    // Autor: David Merfield
    // ----------------------------------------------
    /*
    Se pueden pasar los siguientes parametros para influenciar el color que éste genera:
...
{
    // esta funcion devuelve un color en tono pastel para el fondo de los Grupos 
    // cada que devolvemos un color, lo vamos almacenando en el array de colores
    // y cada que vamos a generar un color, intentamos hasta que devuelva un color 
    // que no se haya utlizado ya
    pastel = generarColorPastel();
    while(colores.indexOf(pastel) >= 0)
    {
       pastel = generarColorPastel();
    } 
    colores.push(pastel);
...
function seleccionarBotonTipoValor(campo, idBoton)
{
    ...
    // Titulo           Permite digitar un texto fijo en el campo
    ...
    switch($("#"+campo).val()) 
    {
        case 'Puc':
            ...
            $("#valorNIIFConcepto").attr('readonly',false);
            $("#excluirTerceroConcepto").attr('readonly',false);
            $("#valorConcepto").attr('placeholder','Ingrese los números de cuenta Local');
            $("#valorNIIFConcepto").attr('placeholder','Ingrese los números de cuenta NIIF');
            ...
            $("#valorNIIFConcepto").attr('title',"Para digitar rangos de cuentas, separelas por guion (1005-10059999)\npara cuentas independientes separelas por coma (10050110,10050111,10050112)\npara excluir cuentas ingreselas comenzando por la letra x (x10050105)");
            $("#valorConcepto").focus();
            break;
        ...
        case 'Valor':
            ...
            $("#valorConcepto").attr('placeholder','Ingrese el valor fijo para Local');
            $("#valorNIIFConcepto").attr('placeholder','Ingrese el valor fijo para NIIF');
            ...
            $("#valorNIIFConcepto").attr('title','El valor puede contener decimales separados por punto, no debe incluir signos de moneda ni caracteres alfabeticos');
            $("#valorConcepto").focus();
            break;
        ...
        case 'Porcentaje':
            ...
            $("#valorNIIFConcepto").attr('readonly',false);
            $("#excluirTerceroConcepto").attr('readonly',true);
            $("#valorConcepto").attr('placeholder','Ingrese el porcentaje para Local');
            $("#valorNIIFConcepto").attr('placeholder','Ingrese el porcentaje para NIIF');
            ...
            $("#valorNIIFConcepto").attr('title','El porcentaje debe digitarse como valor entre 1 y 100, puede contener decimales separados por punto, no debe incluir el signo de %');
            $("#valorConcepto").focus();
            break;

        case 'Titulo':
            $("#valorConcepto").attr('readonly',true);
            $("#valorNIIFConcepto").attr('readonly',true);
            $("#excluirTerceroConcepto").attr('readonly',true);
            $("#valorConcepto").attr('placeholder','No habilitado en modo de título');
            $("#valorNIIFConcepto").attr('placeholder','No habilitado en modo de título');
            $("#valorConcepto").attr('title','No habilitado en modo de título');
            $("#valorNIIFConcepto").attr('title','No habilitado en modo de título');
...
function cargarEstiloInforme()
{
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            var selectorEstilo = '<input type="hidden" id="idCampo" value="">';
        	var tabla = '<table class="table table-striped" style="width:100%">'+
			    '<tbody>';
            ...
            { 
                ...
                selectorEstilo += '<div class="botonEstilo" '+estiloSeleccion+' id="'+data[i].idEstiloInforme+'" onclick="asignarEstiloControl(this);" title="'+data[i].nombreEstiloInforme+'">'+data[i].nombreEstiloInforme+'</div>';
            ...
            $(".containerSelectorEstilo").html(selectorEstilo) ; 
			$("#estilos").html(tabla) ; 
...
function asignarEstiloControl(objSelector)
{
    ...
    var idDiv = $("#idCampo").val(); //$(objSelector).closest('ul').parent().parent().attr('id');
    ...
    $("#Etiqueta-"+idDiv).attr('style',($(objSelector).attr('style')+'width:100%; height:100%'));
    $("#Campo-"+idDiv).attr('style',($(objSelector).attr('style')+'width:100%; height:100%'));

    $(".selectorEstilo").modal("hide");
}
            ...
            success: function(data){
                ...
                for(var i=0; i < data.length; i++)
                {
                    adicionarCapa(data[i].tipoInformeCapa, data[i].idInformeCapa, 
                        ...
                        data[i].tablaInformeCapa, data[i].tituloInformeCapa);
                }
...
function adicionarCapa(tipo, idInformeCapa, accion, idSistema, tabla, titulo)
{
    totalCapas++;
    totalGrupos[totalCapas] = 0;
    ...
    $("#tabcapa").append('<li id="hojacapa'+totalCapas+'" >'+
                                ...
                                '<input type="hidden" id="tituloInformeCapa'+totalCapas+'" value="'+titulo+'">'+
                                '<a data-toggle="tab" href="#capa'+totalCapas+'" onclick="seleccionarCapa('+totalCapas+');">'+
                                        '<input type="hidden" id="tituloCapa'+totalCapas+'" value="'+titulo+'" placeholder="Título" style="display:inline-block;">'+
                                        '<div  style="display:inline-block;" id="etiquetaCapa'+totalCapas+'" ondblclick="cambiarTituloCapa('+totalCapas+')">'+titulo+'</div>'+
                                        '&nbsp;&nbsp;&nbsp;'+
                                        '<button class="close" style="display:inline-block; color:red;" type="button" title="Eliminar esta Capa" onclick="eliminarCapa('+totalCapas+');">×</button>'+
                                '</a>'+
                        '</li>');
    ...
    $("#contentcapa").append(
                    ...
                    '   <div id="layoutEncabezado'+totalCapas+'" class="panel panel-primary panelLayout"  style="height: 100px;">'+
                    ...
                    '           </div>'+
                    '     </div>'+
                    '     <div id="layoutEncabezadoContenedor'+totalCapas+'" class="panel-body contenedorCampos Banda" style="height: 58px; overflow: auto;">'+
                    '     </div>'+
                    '   </div>'+
                (tipo == 1 
                ?
                    '   <div id="EncabezadoGrupo'+totalCapas+'">'+
                    '   </div>'+
                ...
                :
                    ...
                    '   <div id="layoutPie'+totalCapas+'" class="panel panel-primary panelLayout" style="height: 100px;">'+
                    ...
                    '           </div>'+
                    '     </div>'+
                    '     <div id="layoutPieContenedor'+totalCapas+'" class="panel-body contenedorCampos Banda" style="height: 58px; overflow: auto;"'+
                    '     </div>'+
                    '   </div>'+
                    '</div>');

    // ejecuta la funcionalidad de arrastrable que aplica para todos los campos contenedores (contenedorCampos)
    hacerArrastrable();

    // ejecutamos la carga de grupos en la capa creada
    cargarInformeGrupo(idInformeCapa, accion);
...
// en el evento Doble Click habilitamos la funcionalidad propia del objeto de titulo de capa que es permitirle al 
// usuario digitar el texto y que el sistema lo ponga fijo en el div
function cambiarTituloCapa(numCapa)
{
    // lo que hacemos es ocultar el texto del div y mostrar el campo input para que se digite el nuevo texto
    $("#etiquetaCapa"+numCapa).css('display',  'none');
    $("#tituloCapa"+numCapa).prop('style', 'width: 100%; height: 100%;');
    $("#tituloCapa"+numCapa).prop('type','text');
    $("#tituloCapa"+numCapa).prop('title','Ingrese el Titulo de la Capa');
    $("#tituloCapa"+numCapa).focus(); 

    // luego de que el usuario digite el nuevo texto, deberá salir del campo para que se ejecute el evento BLUR
    $("#tituloCapa"+numCapa).blur(function() 
    {
        // en este hacemos lo contrario, ocultamos el input y mostramos el div y le asignamos el texto del input al div fijo
        $(this).prop('type','hidden');
        idEtiqueta = "#"+$(this).prop('id').replace('tituloCapa','etiquetaCapa');
    ...
    .droppable(
    {
       drop: function(e, ui)
       {
            if(ui.draggable.hasClass("campoClonable")) 
            {
                ...
                $( "#item-"+counts[0] ).html( 
                    '<input type="hidden" id="Clave-item-'+counts[0]+'" value="0" >'+
                    ...
                    '       <ul class="dropgroup">'+
                    '         <li class="dropdown">'+
                    '           <a onclick="$(\'#idCampo\').val(\'item-'+counts[0]+'\'); $(\'.selectorEstilo\').modal(\'show\');"  class="dropbtn"><span  class="fa fa-paint-brush" ></span></a>'+
                    '           <div class="dropdown-content selectorEstilo2">'+
                    '           </div>'+
                    '         </li>'+
                    '       </ul>'+
...
function cargarInformeGrupo(idInformeCapa, accion)
{
    var token = document.getElementById('token').value;
    $.ajax({
        headers: {'X-CSRF-TOKEN': token},
        dataType: "json",
        url:   'http://'+location.host+'/consultarInformeGrupo',
        ...
        success: function(data){
            ...
            for(var i=0; i < data.length; i++)
            {
                ...
                adicionarGrupo(data[i].numeroCapa);
            }
...
function adicionarGrupo(numerocapa)
{
    colorGrupo = colorPastel();
    $('#EncabezadoGrupo'+numerocapa).append( 
        '   <div id="layoutGrupoEnc'+numerocapa+'_'+totalGrupos[numerocapa]+'" class="panel panel-primary panelLayout" style="height: 100px;">'+
        '     <div class="panel-heading">Encabezado de Grupo '+totalGrupos[numerocapa]+
        '           <div style="float: right;">'+
        '               <span class="glyphicon glyphicon-chevron-up" title="Reducir Altura" onclick="cambiarTamañoPanel(\'layoutGrupoEnc\',\''+numerocapa+'_'+totalGrupos[numerocapa]+'\', \'R\');"></span>'+
        '               <span class="glyphicon glyphicon-chevron-down" title="Aumentar Altura" onclick="cambiarTamañoPanel(\'layoutGrupoEnc\',\''+numerocapa+'_'+totalGrupos[numerocapa]+'\', \'A\');"></span>'+
        '           </div>'+
        '     </div>'+
        '     <div id="layoutGrupoEncContenedor'+numerocapa+'_'+totalGrupos[numerocapa]+'" class="panel-body contenedorCampos Banda" style="height: 58px; overflow: auto; background-color: '+colorGrupo+'">'+
        '     </div>'+
        '   </div>');

    $('#PieGrupo'+numerocapa).html( 
        '   <div id="layoutGrupoPie'+numerocapa+'_'+totalGrupos[numerocapa]+'" class="panel panel-primary panelLayout" style="height: 100px;">'+
        '     <div class="panel-heading">Pie de Grupo '+totalGrupos[numerocapa]+
        '           <div style="float: right;">'+
        '               <span class="glyphicon glyphicon-chevron-up" title="Reducir Altura" onclick="cambiarTamañoPanel(\'layoutGrupoPie\',\''+numerocapa+'_'+totalGrupos[numerocapa]+'\', \'R\');"></span>'+
        '               <span class="glyphicon glyphicon-chevron-down" title="Aumentar Altura" onclick="cambiarTamañoPanel(\'layoutGrupoPie\',\''+numerocapa+'_'+totalGrupos[numerocapa]+'\', \'A\');"></span>'+
        '           </div>'+
        '     </div>'+
        '     <div id="layoutGrupoPieContenedor'+numerocapa+'_'+totalGrupos[numerocapa]+'" class="panel-body contenedorCampos Banda" style="height: 58px; overflow: auto; background-color: '+colorGrupo+'">'+
        '     </div>'+
        '   </div>'+
         $('#PieGrupo'+numerocapa).html());

    totalGrupos[numerocapa]++;
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            for(var i=0; i < data.length; i++)
            {
                ...
                valores[11] = data[i].nombreNIIFInformeConcepto;
                valores[12] = data[i].valorNIIFInformeConcepto;
            ...
            for(var i=0; i < data.length; i++)
            {
                ...
                if(data[i].tipoInformeObjeto == "CampoClon") 
                {
                    ...
                    objeto = '<div id="item-'+counts[0]+'" class="item-'+counts[0]+' campoTamaño-'+counts[0]+' '+data[i].tipoInformeObjeto+'" style="'+estilo+'" title="" >'+
                                '<input type="hidden" id="Clave-item-'+counts[0]+'" value="'+data[i].idInformeObjeto+'" >'+
...
function adicionarConcepto(numeroCapa, valores)
{
    ...
    if(valores[1] == 'MovimientoContable')
    ...
    switch(valores[2])
    {
        ...
        case 'Porcentaje':
            ...
            break;    
        case 'Titulo':
            iconoTipoValor = 'src="http://'+location.host+'/imagenes/texto.png"';
            break;      
        default:
            iconoTipoValor = 'src="http://'+location.host+'/imagenes/abaco.png"';
            break;
    ...
    $("#sortable"+numeroCapa).append(
            ...
            '       <input type="text" id="nombreConcepto_'+numeroCapa+'_'+totalConceptos+'" name="nombreConcepto[]" value="'+valores[0]+'" title="Descripción del Concepto Local" class="campoConcepto" style="'+valores[4]+'" readonly="readonly">'+
            '       <input type="hidden" id="nombreNIIFConcepto_'+numeroCapa+'_'+totalConceptos+'" name="nombreNIIFConcepto[]" value="'+valores[11]+'" title="Descripción del Concepto NIIF" class="campoConcepto" style="'+valores[4]+'" readonly="readonly">'+
            ...
            '       <input type="text" id="valorConcepto_'+numeroCapa+'_'+totalConceptos+'" name="valorConcepto[]" value="'+valores[3]+'" title="Contenido del Concepto Local" class="campoConcepto"  style="'+valores[4]+'" readonly="readonly">'+
            '       <input type="hidden" id="valorNIIFConcepto_'+numeroCapa+'_'+totalConceptos+'" name="valorNIIFConcepto[]" value="'+valores[12]+'" title="Contenido del Concepto NIIF" class="campoConcepto"  style="'+valores[4]+'" readonly="readonly">'+
...
function MostrarConcepto(accion, numeroConcepto)
{
    ...
    $('#nombreNIIFConcepto').prop('readonly', false);
    $('#valorConcepto').prop('readonly', false);
    $('#valorNIIFConcepto').prop('readonly', false);
    ...
    {
        $('#nombreConcepto').val('');
        $('#nombreNIIFConcepto').val('');
        $('#tipoMovimientoConcepto').val('MovimientoContable');
        $('#tipoValorConcepto').val('Puc');
        $('#valorConcepto').val('');
        $('#valorNIIFConcepto').val('');
    ...
    else
    {
        ...
        $('#nombreNIIFConcepto').val($("#nombreNIIFConcepto_"+numeroConcepto).val());
        ...
        $('#valorNIIFConcepto').val($("#valorNIIFConcepto_"+numeroConcepto).val());
        ...
        if(accion == 'eliminar')
        {
            ...
            $('#nombreNIIFConcepto').prop('readonly', true);
            $('#valorConcepto').prop('readonly', true);
            $('#valorNIIFConcepto').prop('readonly', true);
    ...
    if($('#tipoMovimientoConcepto').val() == 'MovimientoContable')
    {
    ...
    $("#iconoTituloConcepto").attr('style','background-color: white;'); 
    ...
    switch($('#tipoValorConcepto').val())
    {
        ...
        case 'Porcentaje':
            ...
            break;
        case 'Titulo':
            $("#iconoTituloConcepto").attr('style','background-color: #A9E2F3;'); 
            break;
    }
    ...
    // pone titulo y estilo al boton de grabar (Insertar, Modificar, Eliminar)
...
function modificarConcepto()
{
    ...
    $("#nombreNIIFConcepto_"+numeroCapa).val($('#nombreNIIFConcepto').val());
    ...
    $("#valorNIIFConcepto_"+numeroCapa).val($('#valorNIIFConcepto').val());
    ...
    $("#nombreNIIFConcepto_"+numeroCapa).attr('style', $('#conceptoEstilo option:selected').attr('style'));
    $("#valorConcepto_"+numeroCapa).attr('style', $('#conceptoEstilo option:selected').attr('style'));
    $("#valorNIIFConcepto_"+numeroCapa).attr('style', $('#conceptoEstilo option:selected').attr('style'));
    $("#excluirTerceroConcepto_"+numeroCapa).attr('style', $('#conceptoEstilo option:selected').attr('style'));
    
    // boton de tipo Movimiento
    
    if($("#tipoMovimientoConcepto_"+numeroCapa).val() == 'MovimientoContable')
    ...
    switch($("#tipoValorConcepto_"+numeroCapa).val())
    {
        ...
        case 'Porcentaje':
            ...
            break;
        case 'Titulo':
            $("#iconoTipoValorConcepto_"+numeroCapa).attr('src', 'http://'+location.host+'/imagenes/texto.png');
            break;
    }
...
function eliminarConcepto()
{
    ...
    {
        $("#conceptosEliminados").val( 
            $("#conceptosEliminados").val() +
                ($("#idInformeConcepto_"+numeroCapa).val() != 0 
                ? $("#idInformeConcepto_"+numeroCapa).val()+',' 
                : ''));
...
function OcultarConcepto(accion)
{
    ...
    valores[11] = $('#nombreNIIFConcepto').val();
    valores[12] = $('#valorNIIFConcepto').val();
...
function MostrarFormula(accion)
{
    ...
    // con el numero de la capa, consultamos los conceptos de esta capa y los ponemos en la 
    // lista de seleccion de la formula
...
function OcultarFormula(accion)
{
    ...
    if(accion != 'cancelar')
    {
        ...
        $("#valorNIIFConcepto").val($("#formulaconcatenada").val());
    }
...
{
    var conceptosEliminados = $('#conceptosEliminados').val();
        ...
        $(this).find( "div[class*='Banda']" ).each(function()
        {
            ...
            $( this ).contents().find( "[id^='concepto']" ).each(function()
            {
               ...
               datoObjeto +=    'concepto|'+
                                ...
                                $("#nombreNIIFConcepto"+numeroConcepto).val() + '|' +
                                $("#valorNIIFConcepto"+numeroConcepto).val() + '|' +
                                idCapa+'¬';
            });
    ...
    if(accion != 'cancelar')
    {
        ...
        $.ajax({
            ...
            data: {
                ...
                objetos: datoObjeto,
                conceptosEliminados: conceptosEliminados
                },
            type:  'post',
            beforeSend: function(){
                },
            success: function(data){
                $("#idInforme").val(data);
                //if($("#accionInforme").val() == 'adicionar')
                    window.location.replace("http://"+location.host+"/disenadorinforme?idInforme="+data+"&accion=modificar&modo="+$("#modoInforme").val()+"&idCategoria="+$('#idCategoria').val());

                alert('Datos Guardados con el ID: ' + data);
